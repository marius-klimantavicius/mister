...
namespace FASTER.core
{
    ...
    internal abstract class SynchronizationStateMachineBase : ISynchronizationStateMachine
    {
        ...
        public async ValueTask OnThreadEnteringState<Key, Value, Input, Output, Context>(SystemState current, SystemState prev, FasterKV<Key, Value, Input, Output, Context> faster, CancellationToken token = default(CancellationToken))
            where Key : new()
            where Value : new()
        {
            foreach (var task in tasks)
