...
namespace FASTER.core
{
    ...
    internal class IndexSnapshotTask : ISynchronizationTask
    {
        ...
        public async ValueTask OnThreadState<Key, Value, Input, Output, Context>(SystemState current, SystemState prev, FasterKV<Key, Value, Input, Output, Context> faster, CancellationToken token = default(CancellationToken))
            where Key : new()
            where Value : new()
        {
            switch (current.phase)
